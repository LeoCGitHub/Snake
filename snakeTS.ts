...
function nourriture() {
    cote = 20;
    let xNTemp: number = Math.ceil(Math.random() * 580);
    xN = xNTemp - xNTemp % 20;
    let yNTemp: number = Math.ceil(Math.random() * 580);
    yN = yNTemp - yNTemp % 20;
...
function ennemy() {
    coteE = 20;
    let xETemp: number = Math.ceil(Math.random() * 580);
    xE = xETemp - xETemp % 20;
    let yETemp: number = Math.ceil(Math.random() * 580);
    yE = yETemp - yETemp % 20;
    tabE.push(xE);
    tabE.push(yE);
    console.log(tabE);
...
let c: any = document.getElementById("field");
...
class Snake {
    ...
    affichage() {
        ...
        for (let i: number = this.tab0.length - 1; i >= 3; i -= 2) {
        ...
        for (let j: number = 0; j < this.tab0.length; j += 2) {
            if (j == 0 && j + 1 == 1) {
        ...
        for (let j: number = 0; j < tabE.length; j += 2) {
            ctx.fillStyle = ctx.createPattern(im4, 'repeat');
        ...
        if (clickState === true) {
            ...
            if (abo.getTab()[0] > 580) abo.getTab()[0] = -20;
            else if (abo.getTab()[1] > 580) abo.getTab()[1] = -20;
            else if (abo.getTab()[0] < 0) abo.getTab()[0] = 600;
            else if (abo.getTab()[1] < 0) abo.getTab()[1] = 600;
        }
    }
    ...
    lose() {
        for (let y: number = 2; y < this.tab0.length; y += 2) {
    ...
    eatEnnemy() {
        ...
        for (let y: number = 0; y < tabE.length; y += 2) {
...
let frame: number = 12;
let abo = new Snake();
let fpsInterval: number = 1000 / frame;
let then: any = Date.now();
let startTime: any = then;
...
function keys(eve: any, snake: any) {
    switch (eve.keyCode) {
        case 37:
...
function start() {
    ctx.clearRect(0, 0, innerWidth, innerHeight);
    if ($('#text1').val() !== undefined) frame = $('#text1').val();
    abo = new Snake();
...
let alienState: boolean;

function difficultyMode(this: string) {
    ...
    if ($(this).text() === "Easy" || $(this).text() === "Hardcore") alienState = false;
    else alienState = true;
}
